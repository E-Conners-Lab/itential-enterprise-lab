...
```
 ...
 clab-rtr1 ------------------------- clab-rtr2        C8000v 17.13.01a (vrnetlab, nested KVM)
   ...
   Eth1|                                  | Eth1
 clab-sw1 --------- trunk 10,20,99 ---- clab-sw2      cEOS 4.33.1.1F
...
| `configs/c8000v.cfg.j2`, `configs/ceos.cfg.j2` | Startup configs per kind. The device password comes only from `CLAB_AUTOMATION_PASSWORD` |
...
3. `images/fetch.sh arista`: cEOS, version read from `versions.yaml` (needs `ARISTA_TOKEN`)
...
5. `images/fetch.sh clab-load`: staged images to the VM's `/srv/stage`, then `clab-host.yml` again to import
   cEOS and build `vrnetlab/cisco_c8000v:17.13.01a`
...
- A C8000v first boot takes 10-20 minutes nested; `clab-dev.yml` waits up to 30 minutes for a real login.
...
- No node keeps a vendor default password (PID success criterion 5). vrnetlab's C8000v bootstrap and
  containerlab's cEOS kind both create `admin`/`admin`; the startup configs set `admin`'s secret to
  `CLAB_AUTOMATION_PASSWORD`, the same as `automation`. vrnetlab applies the startup config after its bootstrap
  and never logs in again (`cisco/c8000v/docker/launch.py` at the pinned commit), containerlab's cEOS kind does
  not log in as `admin` after deploy, and every play and verify here uses `automation` (`verify/devcmd.py`).
