...
def test_branch_vlan_rolls_back_a_failure_between_the_reservation_and_the_push_and_only_there() -> None:
    ...
    import json
    wf = json.loads((ROOT / "itential" / "workflows" / "add-branch-vlan.json").read_text())
    tasks, tr = wf["tasks"], wf["transitions"]
...
def test_compliance_report_never_reads_compliant_without_a_checked_device_and_every_call_can_end() -> None:
    ...
    wf = json.loads((ROOT / "itential" / "workflows" / "summarize-compliance-results.json").read_text())
    tasks, tr = wf["tasks"], wf["transitions"]
    for tid, task in tasks.items():
...
def test_show_command_keeps_the_raw_answer_when_the_parser_cannot_be_chosen() -> None:
    ...
    import json
    wf = json.loads((ROOT / "itential" / "workflows" / "run-show-command-on-a-device.json").read_text())
    tasks, tr = wf["tasks"], wf["transitions"]
...
def test_config_push_reads_the_device_reply_and_never_saves_a_refused_push() -> None:
    ...
    wf = json.loads((ROOT / "itential" / "workflows" / "push-configuration-with-approval.json").read_text())
    tr = wf["transitions"]
...
def test_branch_vlan_reads_the_switch_reply_before_netbox_marks_the_vlan_active() -> None:
    """Owner decision 2026-09-24: as in Push Configuration with Approval, send-config's success flag is not trusted for refused lines.
