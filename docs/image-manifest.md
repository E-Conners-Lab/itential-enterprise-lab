...
| Garage (S3 object store) | 2.4.0 (2026-09-06) | Kustomize, single node (`--single-node --default-bucket`) | `docker.io/dxflrs/garage:v2.4.0` (tag confirmed on Docker Hub) | [releases](https://git.deuxfleurs.fr/Deuxfleurs/garage/releases) | Was the CNPG backup target; empty since ADR 0064 (lab CNPG databases are not backed up), available for later Gitea/NetBox. **MinIO is not used**: `minio/minio` was archived 2026-04-25 and `minio/operator` on 2026-03-20 (ADR 0031) |
| CNPG Barman Cloud plugin | 0.15.0 (2026-09-03) | `kubectl apply` of the release manifest into `cnpg-system` | per release manifest | [installation](https://cloudnative-pg.io/plugin-barman-cloud/docs/installation/) | In-tree `barmanObjectStore` is deprecated since CNPG 1.26; the plugin needs cert-manager. Installed but unused since ADR 0064 |
