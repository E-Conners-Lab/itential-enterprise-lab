...
- **Acceptance** (`verify/test-06d-integrations.sh`, second sources `verify/devcmd.py` for the device and the NetBox and ServiceNow APIs for the record):
  ...
  7. Regression: `verify/test-05-itential.sh`, `test-06-flowai.sh`, `test-06b-platform.sh` and `test-06c-netbox.sh` all pass unchanged against the converted environment — the conversion changes how a call is made, never what it does.
