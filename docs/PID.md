...
- **Acceptance** (`verify/test-06d-integrations.sh`, second sources `verify/devcmd.py` for the device and the NetBox and ServiceNow APIs for the record):
  ...
  5. ServiceNow through the integration: a change-managed run of `wf-config-push-v1` opens a change, walks it through its states and closes it, with no `genericAdapterRequest`; the PDI's Table API confirms the states and the work note.
...
- **The task contract, measured on 6.5.2** (S4f probe, 2026-09-10; the platform validates an integration task on import and leaves the workflow a **draft** if it disagrees, naming the fault): an integration task addresses the model by `<title>:<version>` and the instance by `adapter_id`, exactly as an adapter task addresses its export; every input key must be a parameter the operation **declares**, where an adapter accepted any key (`Cannot find match for input: "offset" from task`); and the single output is named **`response`**, not `result` (`Output: "result" does not match model output: "response"`). That output is the HTTP response object — `ok`, `status`, `statusText`, `url`, `method`, `headers`, `text` (the body as a string) and **`body`** (the parsed payload) — so a consumer that read `result.response.<x>` from the adapter reads `response.body.<x>` from the integration.
  A parameter's value is **JSON-decoded before the call**: a task input that is a bare string fails the whole task with `Could not parse parameter value string as JSON Object or JSON Array`, and the job's own `incoming` view shows every parameter as `null` whether the task worked or not, so it is no help in diagnosing this. A number passes untouched. This is what makes the conversion more than a rename: every operation parameter whose value is a string — including every `$var` reference — has to be produced in a form the platform will decode, which is the `Tools.replace` + `Tools.parse` pattern ADR 0036 already needed for nested adapter inputs. Measured 2026-09-10; `wf-netbox-device-count-v1` converts cleanly because its only parameter is the number `1`.
